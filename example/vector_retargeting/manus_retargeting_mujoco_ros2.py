...
class ROS2ManusNodeSubscriber(Node):
    """ROS2 节点，订阅 ManusNodePoses 消息，把 25 点筛成 21x3 塞进 queue。"""
    ...
    def landmark_callback(self, msg: ManusNodePoses):
        try:
            ...
            # 1. 解析 25 点并剔除 5,10,15,20 -> 21 个关键点（单位 m）
            ...
            logger.error(f"Error processing ManusNodePoses: {e}")
...
def main(
    ...
    hand_type: HandType,
    mjcf_path: str,
    ros_topic: str = "/manus_node_poses_0",
    ...
    - 左进：ROS2 ManusNodePoses (25 点 -> 21x3 手部关键点)
